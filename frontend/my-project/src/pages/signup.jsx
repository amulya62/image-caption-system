...
const Signup = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-indigo-950 to-black px-6 py-12 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 -left-20 w-80 h-80 bg-indigo-600/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md glass rounded-3xl p-8 shadow-2xl shadow-black/40 animate-slide-up relative z-10">
        ...
        <p className="text-gray-400 text-center mt-2 mb-8 text-sm">Sign up and start generating AI captions</p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <Field id="signup-name"     label="Full Name"        type="text"     placeholder="Jane Doe"      field="name" />
          <Field id="signup-email"    label="Email"            type="email"    placeholder="you@example.com" field="email" />
          <Field id="signup-password" label="Password"         type="password" placeholder="••••••••"       field="password" />
          <Field id="signup-confirm"  label="Confirm Password" type="password" placeholder="••••••••"       field="confirm" />

          <button
            id="signup-submit"
            type="submit"
            disabled={loading}
            className="w-full py-3 mt-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 disabled:text-gray-400 transition duration-300 font-semibold text-white flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/25"
          >
            {loading ? (
              <>
